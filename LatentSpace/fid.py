...
def get_activations_from_files(files, sess, batch_size=50, verbose=False):
    ...
    pred_arr = np.empty((n_imgs,17*17*7))
    ...
    for i in tqdm(range(n_batches)):
        ...
        # print( pred.reshape(batch.shape[0],-1).shape)
        # print(pred[:,:,:,0:7].shape)
        pred = pred[:,:,:,0:7]
        pred_arr[start:end] = pred.reshape(batch.shape[0],-1)
...
def calculate_activation_statistics_from_files(files, sess, batch_size=1024, verbose=False):
    """Calculation of the statistics used by the FID.
    Params:
    -- files      : list of paths to image files. Images need to have same dimensions for all files.
    -- sess        : current session
...
def _handle_path(path, sess, low_profile=False):
    if path.endswith('.npz'):
        ...
        files = list(path.glob('*.jpg')) + list(path.glob('*.png')) +  list(path.glob('*.webp')) 
        print(f'img nums:{len(files)}')
        # files =files[:10000]
        if low_profile:
